...
void slalom_run(int x, int y)
{
//引数の座標x,yに向かって最短走行する

	t_direction glob_nextdir;
	double straight_count=0;
	...
	while((mypos.x != x) || (mypos.y != y)){			//ゴールするまで繰り返す
		...
		switch(get_nextdir(x,y,MASK_SECOND,&glob_nextdir))	//次に行く方向を戻り値とする関数を呼ぶ
		{
			...
			case right:
				straight((SECTION*straight_count)-55,FAST_ACCEL,FAST_SPEED,0.14);
				slalom(90,PI*2*3.3,PI*15,RIGHT,0.14);
				straight(1,FAST_ACCEL,FAST_SPEED,0.14);
				straight_count = 0.5;			//走る直線の距離をリセット
				break;
