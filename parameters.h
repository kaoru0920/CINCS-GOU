...
//物理的なパラメータ
#define TIRE_DIAMETER	(12.8)				//タイヤの直径	[mm]
...
#define REF_SEN_R	976				//マウスを迷路中央に置いた時のセンサの値
#define REF_SEN_L	1325				//マウスを迷路中央に置いた時のセンサの値

#define TH_SEN_R	113				//壁があるか否かの閾値	車体を区画の左へ寄せた時のセンサ値(壁あり)
#define TH_SEN_L	213				//壁があるか否かの閾値	車体を区画の右へ寄せた時のセンサ値(壁あり)
#define TH_SEN_FR	150				//壁があるか否かの閾値	
#define TH_SEN_FL	150				//壁があるか否かの閾値

#define RIGHT_90	TH_SEN_FR
#define	LEFT_90		TH_SEN_FL

#define CONTH_SEN_R	REF_SEN_R*0.50 			//TH_SEN_R //REF_SEN_R*0.50			//制御をかけるか否かの閾値
#define CONTH_SEN_L	REF_SEN_L*0.50 			//TH_SEN_L //REF_SEN_L*0.50			//制御をかけるか否かの閾値
...
#define SPEED_KI	(0.8)				//Iゲイン
#define SPEED_KD	(0.4)				//Dゲイン　
...
#define FAST_SPEED	(0.8)				//最短走行の速度	[m/s]
#define FAST_ACCEL	(1.5)				//最短走行の加速度	[m/s^2]
#define MIN_SPEED	(0.1)				//最低速度		[m/s]

#define TURN_ACCEL	(PI*2*6)			//超信地旋回の加速度	[rad/s^2]
#define	TURN_SPEED	(PI*4)				//超信地旋回の最高速度	[rad/s]
#define TURN_MIN_SPEED	(PI/10.0)			//超信地旋回の最低速度	[rad/s]

#define WAIT_TIME	1				//各動作後の待機時間	[ms]

//迷路関連パラメータ
#define GOAL_X	6		//ゴール座標(x)
#define GOAL_Y	1		//ゴール座標(y)
...
//追加したパラメータ
#define GYRO_CORRECTION	1.0120	//ジャイロの誤差補正用
